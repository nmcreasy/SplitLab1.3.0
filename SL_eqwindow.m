...
%     out.region   = region as given in Global CMT table 
%     
...
% else
  bazi = sl_azimuth(ori(:,1),ori(:,2), eqs(:,1),eqs(:,2))';
  azi  = sl_azimuth(eqs(:,1),eqs(:,2), ori(:,1),ori(:,2))';
  distmp = sl_distance(ori(:,1),ori(:,2), eqs(:,1),eqs(:,2));
  dis = distmp';
% end
...
sks     = find(SKSwin(1)       <  dis       & dis      < SKSwin(2) ...
             ...
             & config.azwin(1) <= bazi & bazi <= config.azwin(2));
...
lest = sprintf('Number of Earthquakes Found %i',length(sks));
msgbox(lest,'Earthquakes')
for i=1:length(sks)         
