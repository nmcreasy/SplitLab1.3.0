%RSAC    Read SAC binary files in non-SUN byte order.
...
%    plot(KATH(:,1),KATH(:,2))
%
%    [SQRL, AAK] = sl_rsac('SQRL.R','AAK.R');
%
%    by Michael Thorne (4/2004)   mthorne@asu.edu

function [varargout] = sl_rsac(varargin);
...
% and sl_rsac cannot read this file in.  To correct, read the SAC file into
...
if (h(77) == 4 | h(77) == 5)
    ...
    error(message)
elseif h(77) ~= 6
    message = strcat('Current sl_rsac byte order: "',endian,'". File: "',sacfile,'" may be of opposite byte-order.');
    error(message)
end
