...
elseif strcmp(evnt.Key,'backspace')
    xlim('auto')
%elseif strcmp(evnt.Key,'rightarrow') % forward one event
%    idx=thiseq.index+1;
%    if idx > length(eq)
%        idx=1;
%    end
%elseif strcmp(evnt.Key,'leftarrow')  % backward one event
%    idx=thiseq.index-1;
%    if idx < 1
%        idx=length(eq);
%    end
...
%     Aniso_multi;
    ...
    switch evnt.Character
        ...
        case 'h'  % esc or home work as well, but this feels more intuitive
            %jump close to selected phase
            val  = get(findobj('Tag','PhaseSelector'),'Value');
            t_home = floor(thiseq.phase.ttimes(val)/10)*10 - 30; %~30 seconds before phase; at full 10 seconds
            xlim([t_home t_home+150]) % timewindow of 150 sec
