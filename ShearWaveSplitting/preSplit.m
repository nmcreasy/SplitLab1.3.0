...
timeE1 = thiseq.Amp.time;

vda=find(timeE1>al);

vdf=find(timeE1<fr);

timeind=vda(1):vdf(end); %indices in that time window

for h=1:length(timeind)
...
%---------------------------------------------------------        
%Splitting Intesity (requires function SplitIntFunction)
% -neala
[silow,SplitInt,sihigh]=SplitIntFunction(Twin,Qwin,thiseq.dt);

%fprintf(' Splitting Intensity = %2.4f \n',SplitInt)
%---------------------------------------------------------
...
% Test to get error bars with corrected traces...
%[errbar_phiRC, errbar_tRC, LevelRC] = geterrorbarsRC(QTcorRC(w), Cmap, Cresult);
%[errbar_phiSC, errbar_tSC, LevelSC] = geterrorbars(QTcorSC(w), Ematrix(:,:,1), Eresult(1));
%[errbar_phiEV, errbar_tEV, LevelEV] = geterrorbars(T(w), Ematrix(:,:,2), Eresult(2));
...
% See also: saveresult.m
    ...
    thiseq.tmpresult.f       = thiseq.f;
        %added this to the code - neala
    thiseq.tmpresult.splint  = SplitInt;
    thiseq.tmpresult.silow   = silow;
    thiseq.tmpresult.sihigh  = sihigh;
