...
fprintf(' %s -- Swapping Q/T components for event  %s:%4.0f.%03.0f (%.0f/%.0f) --',...
...
%% Before filtering, (which I may want to remove) we switch the Q and T components.
% Create temporary vectors to hold data
tmpQ = thiseq.Amp.Radial;
tmpT = thiseq.Amp.Transv;
% Create new Amp sub-structure to contain original
thiseq.Amp.tmpRadial = tmpQ;
thiseq.Amp.tmpTransv = tmpT;
% switch the vectors
thiseq.Amp.Radial = tmpT;
thiseq.Amp.Transv = tmpQ;
