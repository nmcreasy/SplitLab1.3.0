...
% Check for gmt4 versus gmt5
textLines = txt(~isheader & ~isfooter);
gmt5test = sscanf(textLines{1},'%d %d/%d/%d %d %d/%d/%d');
if length(gmt5test) > 3
    gmt5Flag = 1;
else
    gmt5Flag = 0;
end
...
% Allows reading gmt5 style, which uses slashes between integers and adds @
% sign for transparency
if gmt5Flag == 0
    ctable = str2num(strvcat(txt(~isheader & ~isfooter)));
else
    if length(gmt5test) == 8  % no transparency
        ctable=ones(length(textLines),8);
        for iline=1:length(textLines)
            ctable(iline,:) = sscanf(textLines{iline},'%d %d/%d/%d %d %d/%d/%d');
        end
    else   % transparency
        tmpTable=ones(length(textLines),10);
        for iline=1:length(textLines)
            tmpTable(iline,:) = sscanf(textLines{iline},'%d %d/%d/%d@%d %d %d/%d/%d@%d');
        end
        ctable=tmpTable(:,[1:4,6:9]);
    end
...
for iline = 1:length(footer)
    ...
    elseif length(gmt5test) == 8   % no transparency
        if strcmp(footer{iline}(1), 'B')
            bfncol(1,:)=sscanf(footer{iline},'B %d/%d/%d');
        elseif strcmp(footer{iline}(1), 'F')
            bfncol(2,:)=sscanf(footer{iline},'F %d/%d/%d');
        elseif strcmp(footer{iline}(1), 'N')
            bfncol(3,:)=[0 0 0];
            %bfncol(3,:)=sscanf(footer{iline},'N %d/%d/%d');
        end 
    else  % transparency
        if strcmp(footer{iline}(1), 'B')
            tmp=sscanf(footer{iline},'B	%d/%d/%d@%d');
            bfncol(1,:)=tmp(1:3);
        elseif strcmp(footer{iline}(1), 'F')
            tmp=sscanf(footer{iline},'F	%d/%d/%d@%d');
            bfncol(2,:)=tmp(1:3);
        elseif strcmp(footer{iline}(1), 'N')
            %tmp=sscanf(footer{iline},'N	%d/%d/%d@%d');
            %bfncol(3,:)=tmp(1:3);
            bfncol(3,:)=[0 0 0];
        end 
    end
